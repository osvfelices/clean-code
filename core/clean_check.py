...
def rule_debug_output(src: Source, cfg: Config, root: Path) -> Violation | None:
    ...
    hits = [h for h in hits if not re.search(r"\bfile\s*=", h)]
    return Violation("debug-output", "Debug output left behind. Remove it or use the project logger.", hits) if hits else None


def rule_narration(src: Source, cfg: Config, root: Path) -> Violation | None:
    ...
    return Violation("narration-comment", "Comment narrates the edit or restates the code. Delete it; the code says it.", hits) if hits else None


def rule_banner(src: Source, cfg: Config, root: Path) -> Violation | None:
    ...
    for c in src.comments:
        ...
        for k, l in enumerate(c.text.split("\n")):
            if rx.search(l):
                hits.append(hit(src, c.line + k)); break
    return Violation("banner-comment", "Section banner comment. Delete it; use blank lines and function boundaries.", hits) if hits else None


def rule_closing_brace(src: Source, cfg: Config, root: Path) -> Violation | None:
    hits = [hit(src, i + 1) for i, l in enumerate(src.lines) if re.search(r"\}\s*//\s*(end|close|\}|/?\w+\s*$)", l)]
    return Violation("closing-brace-comment", "Closing-brace comment. If the block is too long to track, shorten the block.", hits) if hits else None


def rule_todo(src: Source, cfg: Config, root: Path) -> Violation | None:
    if cfg.allow_todo:
        return None
    hits = grep_comments(src, r"\b(TODO|FIXME|XXX|HACK)\b")
    return Violation("todo-marker", "TODO/FIXME left in delivered code. Do it now, or track it outside the code.", hits) if hits else None
...
def rule_commented_code(src: Source, cfg: Config, root: Path) -> Violation | None:
    hits = []
    for c in src.comments:
    ...
    return Violation("commented-code", "Commented-out code. Delete it; version control remembers.", hits) if hits else None


def rule_redundant_doc(src: Source, cfg: Config, root: Path) -> Violation | None:
    hits = grep_comments(src, r"^\s*(/\*\*?|///|#|\*|//)\s*(gets?|sets?|returns?|creates?|initiali[sz]es?|constructor|default constructor)\s+(the\s+|a\s+)?\w+\.?\s*(\*/)?\s*$")
    return Violation("redundant-docstring", "Docstring restates the signature. Delete it or write what the name cannot say.", hits) if hits else None


def rule_suppressed(src: Source, cfg: Config, root: Path) -> Violation | None:
    ...
    hits += grep_code(src, r"@SuppressWarnings|#pragma warning disable|\[SuppressMessage")
    return Violation("suppressed-check", "A check was silenced. Fix the cause; if the check is wrong, fix the check in its own commit.", hits) if hits else None


def rule_any_type(src: Source, cfg: Config, root: Path) -> Violation | None:
    if src.path.suffix not in TS_LIKE:
        return None
    hits = grep_code(src, r"\bas any\b|:\s*any\b|<any>|\bas unknown as\b|Function\b(?!\s*\()|:\s*object\b")
    return Violation("weak-type", "`any`, `as unknown as`, or a loose type. Write the real type.", hits) if hits else None


def rule_non_null(src: Source, cfg: Config, root: Path) -> Violation | None:
    if src.path.suffix not in TS_LIKE:
        return None
    hits = grep_code(src, r"[\w\)\]]!(?=[\.\)\[,;\s])(?!=)")
    return Violation("non-null-assertion", "Non-null assertion hides a real nullability. Narrow it explicitly.", hits) if hits else None
...
def rule_empty_catch(src: Source, cfg: Config, root: Path) -> Violation | None:
    joined = "\n".join(src.code_lines)
    hits = []
    ...
    return Violation("swallowed-error", "Empty catch/except. Handle it, rethrow with context, or let it propagate.", hits) if hits else None


def rule_secret(src: Source, cfg: Config, root: Path) -> Violation | None:
    hits = [hit(src, i + 1) for i, l in enumerate(src.lines)
    ...
    return Violation("hardcoded-secret", "Credential in source. Read it from the environment or a secret store.", hits) if hits else None


def rule_density(src: Source, cfg: Config, root: Path) -> Violation | None:
    ...
    if code >= 40 and comment > code * cfg.max_comment_ratio:
        return Violation("comment-density", f"{comment} comment lines for {code} code lines. Rename and extract instead of explaining.")
    return None
...
def rule_comment_typography(src: Source, cfg: Config, root: Path) -> Violation | None:
    hits = grep_comments(src, "[\u2014\u2013\u00b7\u2022\u2192\u2713\u2717\u2705\u274c\u26a0\U0001F300-\U0001FAFF]")
    return Violation("comment-typography", "Em dash, middle dot, bullet glyph, emoji or icon in a comment. Plain sentences only.", hits) if hits else None
...
def rule_comment_shouting(src: Source, cfg: Config, root: Path) -> Violation | None:
    ...
    for c in src.comments:
        ...
        for k, l in enumerate(c.text.split("\n")):
            if shouts(l):
                hits.append(hit(src, c.line + k)); break
    return Violation("comment-shouting", "ALL CAPS emphasis in a comment. Say it in a plain sentence.", hits) if hits else None
...
def rule_header_essay(src: Source, cfg: Config, root: Path) -> Violation | None:
    for c in src.comments:
        if c.line > 3:
            break
        body = [l for l in c.text.split("\n") if re.sub(r"^\s*(/\*\*?|\*/?|//|#)\s*", "", l).strip()]
        if len(body) > 4:
            return Violation("header-essay", f"File header comment is {len(body)} lines. At most four: what the module owns and what it deliberately does not. Move detail to the functions.", [hit(src, c.line)])
    return None


RULES = [rule_debug_output, rule_narration, rule_banner, rule_closing_brace, rule_todo, rule_commented_code,
         rule_redundant_doc, rule_suppressed, rule_any_type, rule_non_null, rule_skipped_test, rule_empty_catch,
         rule_secret, rule_density, rule_comment_typography, rule_comment_shouting, rule_phase_label, rule_header_essay]
...
def check_file(path: Path, cfg: Config, root: Path, only_lines: set[int] | None = None) -> list[Violation]:
    ...
    for rule in RULES:
        v = rule(src, cfg, root)
        if not v or v.rule in cfg.disabled:
            continue
        if only_lines is not None:
            # A whole-file rule reports no line, so a targeted edit cannot be what caused it.
            v.hits = [h for h in v.hits if line_of(h) in only_lines]
            if not v.hits:
                continue
        v.hits = v.hits[:MAX_HITS_PER_RULE]
        out.append(v)
    return out
...
SHORT = {
    ...
    "narration-comment": "comment narrates or restates code: delete",
    "banner-comment": "banner comment: delete",
    "closing-brace-comment": "closing-brace comment: delete",
    "todo-marker": "TODO/FIXME: do it or remove",
    "commented-code": "commented-out code: delete",
    "redundant-docstring": "docstring restates signature: delete",
    "suppressed-check": "silenced check: fix the cause",
    "weak-type": "any / loose type: write the real type",
    "non-null-assertion": "`!` assertion: narrow explicitly",
    "skipped-test": "skipped or focused test",
    "swallowed-error": "empty catch: handle or rethrow",
    "hardcoded-secret": "hardcoded secret: use env",
    "comment-typography": "glyph in comment: plain text",
    "comment-shouting": "ALL CAPS in comment: rewrite plainly",
    "phase-label": "task/phase label in comment: remove",
}
...
def format_report(results: dict[Path, list[Violation]], root: Path) -> str:
    ...
    for path, vs in results.items():
        ...
        for v in vs:
            label = SHORT.get(v.rule, v.message)
