...
class Violation:
    rule: str
    hits: list[str]
    # Why the rule could not finish. A rule that could not finish has no hits and is not clean either.
    incomplete: str = ""


class Incomplete(Exception):
    """A rule met something it cannot judge, so its silence would not mean clean."""
...
SECRET_NAME = re.compile(r"(api[_-]?key|secret|passw(or)?d|token|bearer|private[_-]?key)\w*\s*(:\s*string\s*)?[:=]\s*(?P<quote>[\"'])", re.I)


SECRET_VALUE = re.compile(r"[A-Za-z0-9_\-/+=]{12,}")


SECRET_PLACEHOLDER = re.compile(r"process\.env|os\.environ|getenv|\$\{|<[A-Z_]+>|example|placeholder|xxx", re.I)
...
def _debug_output(src, cfg, root):
    if matches_any(src.path, cfg.allow_console, root):
        return []
    # A call anywhere in executable code: an argument, a template substitution, a JSX expression.
    hits = grep_code(src, r"^\s*debugger\b|(?<![\w.$])(?<!def )(console\.(log|debug|info|trace)\(|print\(|pprint\(|dbg!\(|var_dump\(|println!\(|fmt\.Println\(|System\.out\.print|Console\.WriteLine\()")
...
def _closing_brace(src, cfg, root):
    # Read the comment and the code it follows, so a string holding the same text is left alone.
    return [hit(src, c.line) for c in src.comments
            if re.match(r"//\s*(end|close|\}|/?\w+\s*$)", c.text) and src.code_lines[c.line - 1].rstrip().endswith("}")]
...
def _secret(src, cfg, root):
    # The name must be code and the value a whole string literal, so text that only mentions a key is not one.
    hits = []
    for i, code in enumerate(src.executable_lines):
        for name in SECRET_NAME.finditer(code):
            close = code.find(name.group("quote"), name.end())
            value = src.lines[i][name.end():close] if close != -1 else ""
            if SECRET_VALUE.fullmatch(value) and not SECRET_PLACEHOLDER.search(src.lines[i]):
                hits.append(hit(src, i + 1))
                break
    return hits
...
def check_file(path: Path, cfg: Config, root: Path, only_lines: set[int] | None = None) -> list[Violation]:
    ...
    for r in RULES:
        ...
        try:
            ...
            out.append(Violation(r.id, [], incomplete=str(exc)))
            continue
        if not hits:
            continue
